...
| NLQ 回應延遲（P95 Latency） | 人工查詢  > 10分鐘 | < 180 秒 | API Latency Log（P95） | B 分項 NLQ Gateway Log | 每週 |
...
| **4. 測試與驗證服務** | **第三方驗證單位（含物流／冷鏈場域業者）** | **場域端設備布署測試、資料品質驗證、IoT 實測、開關事件準確率（Accuracy）測試** | **436**   | 場域性能測試報告、資料品質驗證報告 |
| **委外合計**       | —                       | —                                               | **2,253** | —                 |
...
| Arviem AG                        | 全球跨境資料、國際場域                      | 實物與資料貢獻     | 580     | 國際合作（不付費）          |
| **台灣物流 / 冷鏈業者（改成：第三方場域測試服務提供者）** | **場域布署測試、資料品質驗證、事件準確率測試、環境壓力測試** | **第三方測試服務** | **100** | 合法可付費（屬測試驗證，不是場域費） |
