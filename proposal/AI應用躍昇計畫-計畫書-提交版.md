...
**B1. 目標與成功標準**
   - 報告製作時間下降 ≥ 50%；NLQ P95 延遲 < 30 秒；正確率 ≥ 90%。
**B2. 取得與準備資料**
...
**B6. 佈署與展示**
   - 上線 NLQ API、Dashboard 與定期報表，提供高階彙報。
...
**C2. 取得與準備資料**
   - 使用溫濕度與位置等資料，搭配路線與季節等背景資訊。
**C3. 建立數據集**
   - 建立多變量資料集與標註，覆蓋主要路線與艙體情境。
**C4. 建模與訓練**
   - 採用穩健的時序預測方法，納入簡單先驗與邏輯檢核。
**C5. 評估與優化**
...
**D1. 目標與成功標準**
   - 偵測 AUC ≥ 0.90、F1 ≥ 0.85；ETA MAE 改善 ≥ 25%；警示誤報率 ≤ 15%；關鍵警示回應時間 < 30 分鐘。
**D2. 取得與準備資料**
   - 使用 GPS 位置、速度、停靠點與外部路況/天候摘要。
**D3. 建立數據集**
   - 以行程切片與異常標註建立資料集，補充路網特徵。
**D4. 建模與訓練**
   - 以時序與地理行為分析識別偏離、可疑停留與未授權開箱，並設定警示優先序。
**D5. 評估與優化**
