...
|---|---|---:|---:|---:|---|
| AWS/Azure 運算資源（GPU 訓練） | 按量計費 | 25 | 18 | 450 | RL模型訓練（DQN/SAC）、LLM微調、異常偵測模型訓練；Phase I–III 分階訓練 |
| 雲端資料庫與儲存（RDS/S3/Blob） | 月租 + 容量 | 12 | 18 | 216 | 時序資料庫、訓練資料集、模型版本管理、場域測試日誌 |
| API服務與LLM推論（OpenAI/Anthropic） | 按量計費 | 8 | 18 | 144 | 自然語言查詢介面、提示工程測試、RAG檢索增強生成 |
| 監控/日誌/APM（DataDog/NewRelic） | 月租 | 6 | 18 | 108 | 系統效能監控、異常警示、日誌分析、可觀測性平台 |
| 容器化部署（K8s/ECS/AKS） | 月租 | 5 | 12 | 60 | Phase II–III 微服務部署、邊緣代理管理、CI/CD管道 |
| 小計 | — | — | — | 978 | 金額單位：千元 |
...
| 項目 | 單位 | 數量 | 單價 | 第1年度 | 用途說明 |
|---|---|---:|---:|---:|---|
| GPU 維護/延伸保固與風扇模組汰換 | 年 | 1 | 400 | 400 | 硬體維護合約與零件汰換 |
| NAS 維護/RAID 硬碟汰換與延伸保固 | 年 | 1 | 300 | 300 | 磁碟模組/控制器維護與保固 |
| 伺服器維護/延伸保固與備品 | 年 | 1 | 300 | 300 | 電源/記憶體/備品汰換 |
| 小計 | — | — | — | 1,000 | 此小計對應彙總表「4.設備維護費」 |
...
| 項目             | 單位 | 數量 | 單價 | 第1年度 | 用途說明 |
|------------------|------|------|------|---------|----------|
| 合作研發與驗證   | 工研院 AI 中心／Arviem AG／台灣–阿姆斯特丹大學 AI 合作中心 | 1 | — | 6,812 | (1) 工研院 AI 中心：強化學習與多智能體模組共同研發、系統性能驗證與測試報告（含外部專家顧問、600K經費支援）。<br>(2) Arviem AG：歐洲場域測試、IoT 冷鏈數據驗證與跨境資料回傳、技術支援。<br>(3) 台灣–阿姆斯特丹大學 AI 合作中心：聯邦學習、AI 倫理與可解釋模型協同研究。<br>(4) 專利申請費用與技術文件編撰。<br>**註：本項目已包含工研院/學研中心600K經費支援。原雲端資源配置過低已調整至合理水平（978K），相應從本科目調減以維持總預算平衡。**<br>共同建立可出口之 SaaS 供應鏈 AI 解決方案。 |
...
| 會計科目 | 補助款 | 自籌款 | 合計 |
|---|---:|---:|---:|
| 1.人事費 | 6,270 | 6,260 | 12,530 |
| 2.消耗性器材及原材料費 | 810 | 810 | 1,620 |
| 3.設備使用費/雲端租賃費 | 1,030 | 1,028 | 2,058 |
| 4.設備維護費 | 500 | 500 | 1,000 |
| 5.技術引進/合作研發費 | 3,410 | 3,402 | 6,812 |
| 6.國內差旅費 | 200 | 200 | 400 |
| 7.專利申請獎勵金 | 80 | 80 | 160 |
| **總開發經費** | **12,300** | **12,280** | **24,580** |
| **百分比** | **50.0%** | **50.0%** | **100%** |
