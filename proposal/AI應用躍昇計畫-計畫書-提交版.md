...
### **Phase II：AI 警示優化與智慧採樣整合（12 個月）— 開始日期：2026/04/01**

**註**：Phase II 與 Phase I 有2個月重疊期（2026/04-05），用於資料管道銜接與模型訓練準備，確保無縫過渡。實際執行期間為 M4-M15（共12個月）。
...
- B1 警示優化與路線異常預測 (15%) – ItracXing 準旺科技（M4-M9）
- B2 IoT 智慧採樣策略優化與部署 (19%) – ItracXing 準旺科技（M4-M11）
- B3 聯合驗證與測試 (5%) – ITRI + 學研中心（M13-M15）

### **Phase III：多智能體部署與國際展示（6 個月）— 開始日期：2027/01/01**

**註**：Phase III 實際執行期間為 M13–M18（2027/01–2027/06），總計畫期為18個月（2026/01/01 - 2027/06/30）。
...
- **2026/04–2026/09（Phase II 前期，M4-M9）**：完成 B1 警示優化與路線異常預測（查核點 B.1）
- **2026/04–2027/03（Phase II 全期，M4-M15）**：完成 B2 IoT智慧採樣策略開發與 B3 聯合驗證與測試，ITRI聯合驗證報告（查核點 B.2）
- **2027/01–2027/06（Phase III，M13-M18）**：完成 C1–C5 多智能體協作、聯邦學習POC、生產部署、API文件、驗收與交付（查核點 C.1；結案驗收）
...
- **A.1（2026/05底）—Phase I結束**：
  ...
  - 自動報告與高階警示彙報系統上線
  - 交付：系統原型、技術文件、測試報告
- **B.1（2026/09底）—Phase II前期結束**：

  - 警示優化模型訓練完成，F1-score≥0.85
  - 交付：模型訓練報告、初步驗證結果
- **B.2（2027/03底）—Phase II結束**：
...
| **Phase II**  | M4-M15  | RL警示 + 採樣優化 + 驗證    | 35%      | 3,150K      | —         |
| **Phase III** | M13-M18 | 多智能體 + 生產部署         | 20%      | 1,800K      | 5% (450K)* |
...
- 三年累計總體產值（SaaS + AI-RAN 帶動，含上中下游）：約 NT$120M
- 出口比重：約 70%；內需貢獻：約 30%
...
| --- | --- | --- | --- | --- | --- |
| 基於強化學習的 IoT 採樣優化方法 | 發明/新型專利 | TW | 2026/05 送件（IDF 2026/03、初稿 2026/04） | ITracXing 準旺科技股份有限公司 | Phase II 前期成果（B2） |
| 可解釋式供應鏈異常偵測系統 | 發明專利 | TW | 2026/06 送件（檢索 2026/04） | ITracXing 準旺科技股份有限公司 | Phase I/II 串接（A2→B1） |
| Multi-Agent Supply Chain Monitoring with Federated Learning | PCT（國際） | PCT | 2026/10 送件（優先自 TW 案） | ITracXing 準旺科技股份有限公司 | Phase III 佈局，延伸 US/EU 進入 |
| 電池健康指標（BHI）計算模組 | 電腦程式著作權 | TW | 2026/02 登記 | ITracXing 準旺科技股份有限公司 | Phase I 產出（A2/A3） |
| 多智能體任務排程器（MCP/Agent Orchestrator） | 電腦程式著作權 | TW | 2026/08 登記 | ITracXing 準旺科技股份有限公司 | Phase II 產出（B1/B2） |
...
- PCT 佈局：M10（2026/10）；進入國別階段於 12–30 月內依市場選擇。
...
- **Phase III（M18）**：完成生產部署與驗收；雲端資源作為最小可用服務（Minimal Viable Service）的託管與監控，不涉及模型訓練。
