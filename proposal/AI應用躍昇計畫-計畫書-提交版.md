...
> 效益衡量分四層：技術成效、營運與採用、產值與市場擴散、ESG 與研發效率。

#### 1. 整體 KPI 架構總覽

| 類別 | KPI 指標 | 說明 | 112 年/現況 | 計畫期末目標值 | 查核方式 | 負責單位 |
| --- | --- | --- | --- | --- | --- | --- |
| 技術成效 | 裝置可用率（Uptime） | 以上線裝置為母體統計 | 約 90% | ≥ 95% | 系統日誌與監控報表 | IoT |
| 技術成效 | 異常偵測 F1 | 標註事件為基準 | 無系統性量測 | ≥ 0.85 | 測試集＋案例驗證 | AI |
| 技術成效 | NLQ 查詢 P95 延遲 | 管理者查詢回應時間 | 人工數小時 | ≤ 60 秒 | 監控與日誌 | 平台 |
| 技術成效 | NLQ 查詢成功率 | 合法結果比例 | 無系統性量測 | ≥ 90% | 使用紀錄／日誌 | 平台＋AI |
| 營運／採用 | 每場域試導入天數 | 每家場域上線運行天數 | 無系統性量測 | 每家 ≥ 45 日（1 國內＋1 國外） | 場域紀錄／日誌 | PMO |
| 營運／採用 | MAU | 月活躍使用者數 | 0 | 期末 ≥ 30 | 帳號／操作日誌 | PMO＋前端 |
| 營運／採用 | 告警回應率 | SLA 內確認與處理比例 | 無系統性量測 | ≥ 70% | 告警處理記錄 | 場域＋PMO |
| 產值／市場 | 累計導入客戶數 | 試導入或付費導入 | 0 | ≥ 10 家 | 合約／LOI／POC | BD |
| 產值／市場 | 累計建置裝置數 | 上線使用之裝置數 | 0 | ≥ 1,000 台 | 出貨／建置記錄 | IoT＋BD |
| 產值／市場 | 累計產值／營收 | 訂閱、設備、顧問等 | 0 | 三年 ≥ NT$120M（期內 NT$30–40M） | 財務報表 | 財務＋BD |
| ESG | 無效運輸里程降低 | 空跑／重送里程比例 | 無系統性量測 | ≥ 10% | 運輸紀錄對照 | PMO＋場域 |
| ESG | 年減碳量（tCO₂e） | 無效運輸／貨損改善 | 無系統性量測 | ≥ 80（累計） | GHG 方法論報告 | PMO＋顧問 |
| 研發效率 | Lead Time | 需求至上線時間 | 8–12 週 | 4–6 週 | 專案工具紀錄 | 技術長＋PMO |
| 研發效率 | AI 開發覆蓋率 | 使用 AI 工具輔助比例 | 少量試驗 | ≥ 50% | Code Review 標記 | 技術長＋AI |

#### 2. 分階段查核點與里程碑對應

| 時點 | 主要里程碑 | 需達成之量化指標（節錄） | 查核文件與方式 |
| --- | --- | --- | --- |
| 第 6 個月 | 資料管線＋第一版儀表板、NLQ 原型與單一場域試運轉 | 資料匯流／清洗；NLQ P95 ≤ 90 秒；測試場域連線 ≥ 30 日 | Demo、測試報告、日誌截圖 |
| 第 12 個月 | 1 國內＋1 國外場域試導入；異常偵測與風險評分試用 | 每場域 ≥ 45 日；F1 ≥ 0.80；MAU ≥ 15；裝置 ≥ 300 台 | 場域紀錄、KPI 截圖、技術報告 |
| 第 18 個月 | 多場域實證與商業模式驗證（期末） | F1 ≥ 0.85；NLQ P95 ≤ 60 秒／成功率 ≥ 90%；客戶 ≥ 10；裝置 ≥ 1,000；期內產值 ≥ NT$30–40M；無效里程 ≥ 10%、年減碳 ≥ 80 tCO₂e | KPI 儀表板、財務報表、成效報告、第三方佐證 |

#### 3. 儀表板與報表追蹤機制

- 指標層儀表板每日自動更新（技術／營運／產值／ESG）。  
- 週報／月報節奏，未達標指標啟動「KPI 改善專案」。  
- 審計與留痕：KPI 計算文件、版本與規則異動留存。

#### 4. 第三方市場基準與產值合理性檢核

- 市場成長基準（SCM／AI in Logistics／冷鏈監控 CAGR 8–36%）。  
- 內部檢核：簽約客戶、裝置數與訂閱收入按年度 Scenario 對比，滾動調整。
