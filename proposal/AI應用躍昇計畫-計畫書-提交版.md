...
| **痛點二｜報告產出耗時且缺乏互動（人工彙整、決策延遲）** | **B. 按需報告與即時查詢（NLQ）** | 自然語言查詢（NLQ）、NL-to-SQL、LLM 報告生成 | 讓管理階層與營運人員可用自然語言下指令取得數據與報告，將報告產出由 **「人工數小時 → 自動數秒」**，並將 NLQ P95 延遲目標調整為 <60 秒、查詢正確率 ≥90%。 |
...
> 註：本表所列指標與前述「技術驗證協議」、「預定進度表與查核點」及里程碑（A.1／B.1／B.2／C.1）一一對應。其中 NLQ P95 延遲 <60 秒 為期末目標值；文中提及之 <60 秒 為 M1–M6 初期壓力測試與 Go/No-Go 參考基準。警示誤報率 ≤15%、環境異常 F1 與路線／竊盜 F1 等數值亦與技術驗證協議之目標設定一致。
...
| NLQ 回應延遲（P95 Latency） | 人工查詢  > 10分鐘 | < 60 秒 | API Latency Log（P95） | B 分項 NLQ Gateway Log | 每週 |
...
| **NLQ P95 延遲**                                     |                           < 60 秒（壓測基準） |             **< 60 秒** | 壓力測試：P95 latency 指標（含 200 並發、1,000 筆查詢） | APM、壓測報告              | 週 / 月   |   |
