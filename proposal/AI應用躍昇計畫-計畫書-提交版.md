...
3. **環境異常偵測與路線異常偵測：分層導入、降低誤報**  
  - 針對溫/濕度敏感貨品，以「變化點＋門檻＋事件對齊」作為 MVP 進行異常偵測與自動化報告；並設定報告 SLA（P95 ≤ 2 分）與提前預警率目標。  
...
**C6. 佈署與展示**
- 異常即時觸發分項計畫 B 的 AI 自動化報表（沿用模板），以 Email/Teams/LINE 推播事件摘要（時間窗、影響批次、對齊證據、建議處置），Dashboard 提供一鍵 drill‑down 與 NLQ 跳轉（C×B 整合）。
...
| 溫/濕度異常偵測與自動化報告 | B.1 | 提前預警率 / 報告SLA | ≥70% / P95≤2分 |
...
gantt
  ...
  section 分項計畫C 環境異常偵測與自動化報告（MVP）
