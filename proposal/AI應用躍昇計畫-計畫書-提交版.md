...
- 根節：AI智慧供應鏈風險預測與通訊整合（總開發費用＝100%）
  - ├─ 分項A：感測器可靠度優化（RUL）〔占比：28%〕
  |   ├─ (2) 執行單位：ItracXing（AI/後端/Edge 韌體）
  |   ├─ (3) 驗證：第三方場域性能測試（SimiQue 等）
  |   └─ (3) 技術引進/委託：必要時與學研中心協作特徵驗證
  - ├─ 分項B：AI自主式報告與NLQ〔占比：18%〕
  |   ├─ (2) 執行單位：ItracXing（NL-to-SQL／Dashboard）
  |   └─ (3) 技術引進：LLM 與查詢網關服務（OpenAI/Anthropic，按量）
  - ├─ 分項C：環境異常偵測（溫/濕度）〔占比：26%〕
  |   ├─ (2) 執行單位：ItracXing（變化點／多變量時序）
  |   └─ (3) 驗證：國內示範場域（倉儲/冷鏈）第三方事件測試
  - ├─ 分項D：路線偏離與竊盜偵測（Smart TOTE）〔占比：23%〕
  |   ├─ (2) 執行單位：ItracXing（GPS/Geo-fence/Padlock）
  |   └─ (3) 國際驗證：Arviem 國際路線實證
  - └─ 技術引進／委託研究／驗證〔占比：5%〕
      ├─ (3) 委託研究：學研中心（FL 架構設計與模型驗證）
      ├─ (3) 合規顧問：GDPR/PIA/AI Act 審查
      └─ (3) 專利與第三方測試：專利代理／場域性能驗證
