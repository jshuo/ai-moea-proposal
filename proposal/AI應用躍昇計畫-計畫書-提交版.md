...
現行感測器多以固定週期回傳資料，缺乏對自身電池健康狀態與剩餘壽命的預測能力。一旦因電量衰退、訊號衰減或環境干擾導致感測器離線，通常都是在資料已中斷、異常已發生後才被動察覺，造成監控盲區，難以追溯問題發生時的狀況，並導致維運成本提高。

以具體情境為例：某智慧倉儲部署了數百個環境感測器監控溫溼度，其中一個感測器的鋰電池因長期運作逐漸衰退，於夏季高溫期間電量驟降導致裝置提前離線。然而直到幾天後管理人員檢查報表時，才發現該區域的資料已中斷，而在感測器離線的這段期間倉庫內溫度曾飆升，導致多批溫度敏感貨品受損。這種事後才察覺的情形說明：缺乏電池健康監控和壽命預測機制，使得感測器一旦沒電故障，就會產生資料缺口而錯失關鍵事件的即時偵測。相較之下，業界領先者已開始為物聯網裝置導入電池健康監測。例如全球航運龍頭馬士基（Maersk）的智慧貨櫃就即時追蹤感測器的電池電量與健康狀態，以確保裝置在長途運輸中持續運作、不會中途斷電。

→ **需求：** 導入電池健康指標（Battery Health Indicator, BHI）與剩餘壽命預測（Remaining Useful Life, RUL）模型，建立可預測的感測器健康管理與預防性維護機制。例如透過機器學習分析電池電壓、溫度和放電曲線，提前預測電池何時可能耗盡，主動通知更換。同時，串接分項計畫 B 的 AI 自主式事件報告，讓系統在感測器異常離線時自動通報並標記最後有效資料點，以協助維運人員迅速介入處置，提升整體系統可用度（此需求對應分項計畫 A）。
