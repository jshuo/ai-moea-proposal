...
- **痛點一｜感測器可靠度不足（電池衰退、環境干擾、資料遺失）**  
  現行感測器多採固定週期回傳，缺乏電池健康與剩餘壽命的預測機制，當感測器因電量不足或環境干擾而離線時，往往在「資料已中斷、異常已發生」後才被發現，不但增加維護成本，也造成監控盲區與風險判讀困難。  
  → **需求**：建立可預測的電池健康與壽命管理機制，導入 BHI/RUL 預測與預防性維護（對應分項計畫 A）。

- **痛點二｜報告產出耗時且缺乏互動（人工彙整、決策延遲）**  
  目前報告流程需跨系統人工彙整感測器時序資料、GPS 軌跡與事件紀錄，常仰賴 Excel 或人工撰寫簡報，不僅需數小時甚至數日，且容易因版本不一致或欄位解讀差異造成決策延遲與誤判。管理階層與營運人員無法以自然語言即時查詢關鍵指標，只能被動等待報表產出。  
  → **需求**：導入 AI 自動化報表與自然語言查詢（NLQ）之互動式平台，縮短報表產出時間並提升資訊即時性（對應分項計畫 B）。
  
- **痛點三｜環境敏感貨品異常風險高（濕度變化難以及時掌握與追溯原因）**  
  對濕度高度敏感的貨品（如咖啡豆、食品原料、精密零組件等），在運輸途中若遭遇艙內濕度長時間偏高或劇烈波動，容易導致發霉、品質劣化與報廢，進而衍生索賠爭議。現行多數系統多為「事後查帳」式查看濕度曲線，缺乏可操作的異常提醒與簡明說明，無法及早處置或快速釐清問題位置與大致成因。  
  → **需求**：先導入一個「濕度異常偵測與簡易說明」功能：結合艙內濕度與箱體開關時間軸，偵測出「長時間超標」與「短時間劇烈波動」兩大類異常，並自動產出簡單標籤（例如：可能與裝櫃前狀態／運輸途中環境變化／頻繁開關箱體相關），搭配標準化事件摘要報告，協助營運人員快速了解異常時間區段與可能影響貨批（對應分項計畫 C）。技術上將採用**時序深度學習模型（如 RNN/LSTM 或 Temporal Fusion Transformer）**搭配**統計式異常偵測與規則引擎**：先以模型預測「合理濕度範圍與趨勢」，再比對實際量測值自動標記異常區段與可能成因，兼顧準確度與可解釋性。  

- **痛點四｜路線繞行與潛在竊盜風險（GPS 偏離、天候與交通影響）**  
  現行多仰賴事後檢視 GPS 記錄或人工作業判斷，難以區分「因天候/塞車造成的合理延誤」與「可疑繞路、異常停留或竊盜行為」。當貨櫃或貨箱遭未授權開啟時，也往往是事後才在倉儲或收貨端被發現，缺乏即時反應空間。  
  → **需求**：建置路線異常與竊盜偵測機制（對應分項計畫 D），結合 GPS/NTN 軌跡、GeoFencing、鎖具/壓力感測，以及天候與交通資訊，以即時辨識異常繞行、可疑停留與未授權開啟事件，並降低誤報率、強化竊盜偵測能力。
...
- **驗證項目（依分項計畫呈現）**
  ...
  - **分項計畫 E（M12–M18）｜跨模組整合與國際輸出驗證**
     - 模組 A–D 跨模組資料同步成功率 ≥ 98%  
       （以每日 1 萬筆記錄比對同步落差）
     - 告警對齊 P95 < 5 秒  
       （以 E2E Latency 實測紀錄驗證）
     - NLQ 整合版延遲 P95 < 60 秒  
       （跨 A–D 模組、壓測 1,000 查詢）
     - 國際展示 ≥ 1 場（如 Arviem / Vector / EU 場域）  
       （附活動紀錄、簡報、簽到或合作函）
     - 國際 LOI／POC ≥ 2 件  
       （附正式函、簡報或信件佐證）
     - 白皮書／技術影片 ≥ 1 份（結案交付）
     - 整體服務可用性（Service Availability）≥ 99.5%  
       （以連續 90 天 SLA 監控紀錄驗證）
     - 系統穩定度（Crash-free Rate）≥ 99%  
       （以後端 Log 與 APM 監控統計）
...
**C4. 建模與訓練**
- 採用 **多變量時序預測模型**：如 **TFT（Temporal Fusion Transformer）**、Prophet、ARIMA/SARIMAX 等，預測未來溫濕度趨勢。
- 將 **外部天候與季節作為外生變數（Exogenous Features）** 納入模型，提升預測準確度。
- 納入簡單先驗（如物理極限值、艙體熱傳導模型）與邏輯檢核，確保預測結果合理性。
- 建立 **違規風險評分機制**，結合預測值、可信區間與歷史違規模式，提前發出預警。
...
- 2026/04–2026/09（M4–M9；對應里程碑 B.1）  
  - 目標：分項計畫C／D 中期模型完成  
  - 主要成果：  
    - 分項C：多變量時序資料集與 TFT 模型完成（C1–C3），環境條件風險預測模型達到預期 AUC/MAE  
...
- 2026/04–2027/03（M4–M15；對應里程碑 B.2）  
  ...
  - 主要成果：  
    ...
    - 分項C：環境條件風險預測在目標場域達成違規提前預警率目標  
    - 分項D：路線異常／竊盜偵測在真實物流路線上完成 A/B 測試，F1 與 ETA MAE 改善達標  
  - 查核點：B.2（2027/03底）

- 2027/01–2027/06（M13–M18；對應里程碑 C.1）  
  - 目標：分項計畫E 國際展示、跨分項整合交付與驗收  
  - 主要成果：  
    - 分項E：完成端到端跨模組整合 Demo，含 Dashboard、MCP Agents、資料目錄與可解釋性套件  
    - 完成 1–2 場國際展示，產出白皮書、技術影片與商務轉換素材  
    - 實現服務穩定性與可用性目標（可用性 ≥ 99.5%，整合缺陷率連續下降）  
  - 查核點：C.1（2027/06底）
...
| --------- | ----------------- | ---------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---- | -- | --------- | --------------- |
| Jeff Shuo | CIO／計畫主持人         | 碩士 (Univ. of Illinois at Chicago Electrical Engineering and Computer Science, MBA San Diego State Univ. ) | 多智能體協作、AI 安全、嵌入式與 IoT–Edge–Cloud 架構設計、國際場域整合；負責全案技術決策、架構審查與跨分項整合，主導分項 A 之 BHI/RUL 技術方向，協調分項 B 之 NLQ/報表需求與 A/C/D 之資料與模型銜接，並主導分項 E 之國際展示、技術驗證與白皮書規劃。 | 18   | 男  | ITracXing | A、B、C、D、E    |
...
| 林昶睿       | 專案經理／AI 應用規劃      | 學士（國立勤益科大資工系）                            | 具 Python 與影像辨識背景（TensorFlow、YOLO），應用於人臉辨識、情緒分析與年齡預測；具大數據分析與統計建模實務；熟悉 NLP、語音辨識（STT）、LLM 文字生成與 Chatbot，並結合 RL 優化互動；負責分項 B 之 NLQ／自動報告與多智能體 AI 架構設計，協助分項 A/C/D 之模型設計與評估，並與主持人共同支援分項 E（整體 AI 應用規劃、Demo 劇本與國際展示內容統籌）。 | 12   | 男  | ITracXing | A、B、C、D、E    |
| 蔡以民       | 後端工程師             | 碩士（國立勤益科大資工系）                            | 後端系統開發與維運（API 設計、資料庫規劃、伺服器部署），熟悉 TypeScript、Node.js、Golang、FastAPI；具 TensorFlow、OpenCV、Scikit-learn 基礎，能配合 AI 團隊實作服務；熟悉 Docker、Nginx、CI/CD，自動化部署至正式環境；負責分項 A–D 之後端服務與資料管線實作（含 BHI/RUL、NLQ、環境預測、路線／竊盜偵測等 API／ETL），並支援分項 E 之整合測試與上線部署。 | 12   | 男  | ITracXing | A、B、C、D、E    |
| 郭冠宏       | 網頁全端工程師           | 碩士（國立勤益科大資工系）                            | 全端開發（React、Vue.js、Next.js、Node.js、FastAPI），即時儀表板與資料視覺化（Chart.js、D3.js、ECharts）；RESTful API 串接與權限驗證；熟悉 Hugging Face、LangChain、OpenAI API 等 AI 服務整合；負責分項 A–D 之前後端整合、監控介面與 Smart TOTE／風險預警 Dashboard，並主導分項 E 的跨模組展示介面與國際 Demo 網站／管理後台。 | 12   | 男  | ITracXing | A、B、C、D、E    |
| 朱只耘       | Flutter 工程師       | 學士（國立勤益科大資工系）                            | 精通 Flutter（Dart、MVVM、Provider、Riverpod），可獨立開發 iOS/Android App；熟悉 RESTful API、GraphQL、WebSocket 串接；具行動端 AI 整合（對話、推薦、行為預測、語音控制），熟悉 Firebase、SQLite、Hive；負責分項 B–D 之行動端 App 與現場使用者介面（如異常推播、現場處置回報與查詢），並配合分項 E 在展會或國際客戶 PoC 場景中提供行動展示與輕量使用情境。 | 10   | 男  | ITracXing | B、C、D、E       |
| （待聘）      | AI 工程師            | 碩士（AI／資工）                                | 學習排序（Learning-to-Rank）、聯邦學習（Federated Learning）、模型調校與大規模訓練；預計負責中後期 AI 代理訓練與驗證，優先支援分項 C/D 之多場域資料建模與部署，並視需求補強分項 A/B/E 之高階模型優化與維運人力（作為各分項尚未覆蓋能力之備援與擴充）。 | 12   | 女  | ITracXing | C、D（視需求 A/B/E） |
