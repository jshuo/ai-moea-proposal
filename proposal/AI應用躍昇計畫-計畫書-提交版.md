...
| 產業 | 標竿案例複製性；O-RAN/AI-RAN 應用推進；跨域人才培育；國際學研合作 | 案例示範；應用場景擴散；人才 ≥ 6 名；國際合作網絡建立 |
...
| ----------------- | -------------------- | ------------------ | ----------------------------------------------------------- | ----------------------------------------------------- |
| **IBM Maximo**  | 規則引擎 + 基礎ML  | 大型企業資產管理 | • 高昂授權費<br>• 客製化成本高<br>• 缺乏智能優化       | ✓ AI-MaaS彈性定價<br>✓ 自適應採樣<br>✓ 可解釋AI   |
...
| **Samsara**     | IoT車隊管理        | 物流與運輸       | • 單一垂直領域<br>• 警示智能化不足<br>• 無多智能體協作 | ✓ 跨場域整合<br>✓ 動態警示優化<br>✓ 多智能體架構   |
| **傳統ERP+IoT** | SAP/Oracle擴充模組 | 既有ERP延伸      | • AI能力薄弱<br>• 即時性差<br>• 誤報率高               | ✓ AI原生設計<br>✓ <2秒查詢<br>✓ 警示誤報率≤15%（含天候/交通過濾）      |
...
- **我方創新**：學習排序（如 LambdaMART/XGBoost）與成本敏感分類，根據歷史回應與誤報成本動態優先級
- **驗證指標**：警示誤報率-30%（含天候/交通過濾）、關鍵警示回應時間<30分鐘、F1-score≥0.85
- **商業價值**：降低人力成本40%，提升客戶滿意度

**c. 雲端自適應採樣優化（全球罕見）**

- **現有技術瓶頸**：IoT設備固定採樣頻率，電池壽命6個月需頻繁更換
- **我方創新**：透過雲端自適應策略（變化率觸發、CUSUM/Changepoint 檢測、Kalman 不確定度）優化採樣，IoT 裝置接收並執行優化後的採樣參數
...
- 國內專利2件：「事件觸發與變異度驅動之 IoT 採樣優化方法」、「可解釋式供應鏈異常偵測系統」
- 國際專利1件（PCT）：「Multi-Agent Supply Chain Monitoring with Federated Learning」

**技術護城河**：

- 核心演算法閉源（採樣決策函數與門檻設計、Kalman 自適應參數）
- IoT韌體加密與安全通訊機制
...
**E3. 主要工作項目（Work Packages, WPs）**
   ...
   - E3-3 白皮書與標準化：方法、指標、驗證協議與復現步驟；對齊 EU AI Act 與 O-RAN/AI-RAN 延伸參與。
...
| ---------------------------------- | ------------------------ | -----------------: | ------------------------------------------------------------------------------------------ |
| AI-RAN/O-RAN 測試與驗證合作 | 外部研究機構 |            3,000 | 與外部研究機構合作進行 AI-RAN/O-RAN 範疇的試煉場域部署及性能驗證，建置測試節點與 AI 加速伺服器 |
...
本計畫結案後三年內，將透過與外部研究機構合作之 AI-RAN/O-RAN 延伸投資，建立可商轉之 AI 邊緣運算網路平台，形成智慧物流監控與 5G 低延遲運算整合之新興應用場域，作為後續國際輸出與標準化推動的基礎。該合作亦將作為 AI-RAN 國產化生態系的示範案例，強化產業鏈結與國際合作潛力。
...
| d. O-RAN/AI-RAN 專案/授權收入 | $0                       | $60,000                   | $200,000                  | $200,000                  |
| **總收入=a+b+c+d（USD）**          | **$93,000**              | **$812,000**              | **$1,342,000**            | **$1,760,000**            |
| **ARR（NT$，以匯率 1:32 計算）**   | **2.98M**                | **25.98M**                | **42.94M**                | **56.32M**                |

註：O-RAN/AI-RAN 收入 (d) 依 2027：2 × $30,000、2028：8 × $25,000、2029：10 × $20,000 估算；已據此重新計算總收入與 ARR。
...
| O-RAN/AI-RAN 專案/授權收入 (d) | $0          | $60,000      | $200,000       | $200,000       |
