...
<mark>為了區分「僅具相關性」的路線特徵與「真正具有因果影響」的高風險因子（例如無故停留、繞路、溫度異常變化與實際竊盜/貨損事件之關係），本計畫於分項計畫 D 中將導入**因果推論模型**（例如 Causal Forests、Double Machine Learning 與結構性因果模型 Structural Causal Models, SCM），用以估計各類事件對竊盜/貨損風險的邊際因果效果，協助業者優先調整真正具因果影響力的路線規劃與作業流程，而非僅依賴相關性指標。
