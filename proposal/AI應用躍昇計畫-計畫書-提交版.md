...
| 環境異常偵測 F1 | ≥0.88 | Rule-based baseline F1 ≈0.70；加入變化點 + 去噪（+0.08≈0.10）＋事件對齊（再 +0.06≈0.08）→ 0.70 +0.16≈0.18 ≈ 0.86–0.88 → 取上限可達值 | 去噪與對齊能獨立提升且誤差不疊加 | 標註事件集；F1 計算 |
| 路線偏離偵測 AUC | ≥0.90 | 純 GPS 模型 AUC 0.82–0.85；加入路網圖特徵與天候/交通上下文提升 ≈0.04–0.06 → 0.86–0.91 → 取 0.90 | 上下文特徵具穩定性；樣本量 >1,000 軌跡 | 測試集 ROC；季度驗證 |
| 竊盜/未授權開啟 F1 | ≥0.85 | BLE Padlock + 壓力/光感事件 → 正負樣本平衡後預期 Precision ≈0.87、Recall ≈0.83 → F1 ≈0.85 | 有足夠模擬與場域實測開啟事件（≥50 次） | 事件混淆矩陣 |
...
#### 5. 風險緩衝
所有目標採「保守可達」：若任一模型在中期 (M9–M10) 評估低於預期（例如 F1 <0.83），啟動備援策略（特徵增強 / 閾值調教 / 模型混合）確保結案前達標；商業指標以 LOI/POC 滾動更新，未達時加速第二渠道拓拓展。
