...
針對全球供應鏈監控之「警示過載/誤報」、「多感測器資料品質不一致」、「缺乏預測性決策」等痛點，本計畫整合可解釋AI、強化學習（RL）與多智能體技術，打造即時異常偵測與警示優化平台，並透過雲端/伺服器端RL訓練優化IoT裝置自適應採樣策略以延長電池壽命。結合瑞士 Arviem 場域與工研院/學研單位驗證，目標降低誤報、縮短回應時間並提升營運效率。

### 計畫結案時預期效益（重點列舉）

- 技術：異常偵測準確率≥90%，電池壽命+40%～60%。
...
供應鏈AI監控、強化學習、可解釋AI、多智能體、IoT智能採樣
...
| ----------------- | -------------------- | ------------------ | ----------------------------------------------------------- | ----------------------------------------------------- |
| **IBM Maximo**  | 規則引擎 + 基礎ML  | 大型企業資產管理 | • 高昂授權費<br>• 客製化成本高<br>• 缺乏智能優化       | ✓ SaaS彈性定價<br>✓ RL自適應採樣<br>✓ 可解釋AI   |
...
- **我方創新**：透過雲端/伺服器端多智能體RL框架訓練最佳採樣策略，IoT裝置接收並執行優化後的採樣參數
...
| 強化學習警示優化 | 誤報高、無學習能力         | DQN 誤報≤15%、關鍵警示<30分鐘           |
| LLM自然語言查詢  | 需SQL與IT支援              | 高層即時查詢、附行動建議                 |
| IoT智能採樣優化  | 固定採樣、壽命6個月        | 雲端RL訓練自適應策略、壽命10–14個月     |
...
- **驗證項目**：
  - Phase I（5個月）：異常偵測準確率、NL查詢延遲、降噪SNR。
  - Phase II（7.5個月）：警示誤報率、電池壽命、採樣策略優化效果。
  - Phase III（6個月）：多智能體吞吐、聯邦學習隱私、穩定性。
- **導入方式**：API整合（Arviem）、雲端部署、分階訓練上線。

### （2）計畫工作項目實施方式（含百分比/單位）

### **Phase I: AI驅動互動即時報告服務（Arviem 客戶） （5個月）— 開始日期：2026/01/01**

(1) 電池壽命與感測器可靠度
Arviem 感測器整合多種環境量測功能，現有可靠度約 90%，但主要失效原因為電池耗竭與溫度變化。電池放電曲線不穩、高溫加速衰減，且缺乏統一的**電池健康指標（BHI）**與壽命預警機制，導致維運單位無法提前預測或規劃更換。

(2) 客戶報告與資料查詢效率
目前週報／月報多由人工彙整，耗時且缺乏互動性。客戶無法即時查詢貨況與電池狀態，也缺乏自動化產出高階管理報告的能力，造成決策延遲與資訊不透明。

### **執行重點與指標**

1. **目標與成功標準**

   - 建立多來源電池監測資料庫，覆蓋 ≥95% 感測器上傳資料。
   - 提供即時可視化介面與查詢服務
   - 建立電池壽命關鍵指標 (Battery Health Index, BHI)，涵蓋溫度、容量與電壓變化。
   - 週/月報自動生成，減少人工報告製作時間 ≥ 50%。
2. **資料取得與整合**

   - 整合電池電壓 (V)、剩餘容量 (%)、溫度 (°C) 等資料。
   - 導入初步清理與平滑機制（如移動平均與 Kalman filter）。
   - 產生每日電池容量變化率 (ΔCapacity/Δt)、平均電壓曲線與溫度分布。
3. **即時監測與視覺化系統開發**

   - 建立電池壽命即時監測儀表板，顯示每顆感測器的剩餘壽命估計與異常指標。
   - 提供自然語言查詢介面（例如：「這週哪些感測器電池壽命低於 30%？」）。
   - 具備電池電壓-溫度交互圖表、壽命衰減曲線、傳輸頻率分布等分析功能。
4. **自動化報告與警示機制**

   - 透過系統資料庫整合與指標運算機制，自動產出高階管理週報／月報。
   - 透過 MCP 與資料庫排程整合，支援 Email 通知或 Dashboard 推送。

- 新增「高階主管優先警示彙報（Executive Priority Alert Report）」：針對重大異常、SLA 風險與營運影響，按日／週自動彙整 Top N 事件、根因摘要、建議行動與責任單位，提供 Email 與儀表板專用版即時推送。

5. **測試與展示**
   - 測試資料完整率、查詢延遲、BHI 更新頻率。
   - 展示「電池壽命即時看板」與自動報告生成流程。

> <mark>註：Phase I不包含AI模型訓練，僅進行資料整合、查詢系統原型與功能驗證，AI模型訓練與優化於Phase II進行。</mark>

此工作包將大幅提升報告產出效率（包含即時查詢與定期報告），強化客戶即時決策能力，並支援多元化管理需求。

- A1 自然語言查詢 (8%) – ItracXing 準旺科技
- A2 電池異常偵測 (10%) – ItracXing 準旺科技
- A3 Kalman降噪 (5%) – ItracXing 準旺科技
- A4 MCP資料庫存取/查詢整合 (4%) – ItracXing 準旺科技
- A5 AI Agent與MCP整合支援定期管理報告 (5%) – ItracXing 準旺科技
- A5 AI Agent與MCP整合支援定期管理報告與高階主管優先警示彙報 (5%) – ItracXing 準旺科技
...
### **Phase II：AI 警示優化與智慧採樣整合（12 個月）— 開始日期：2026/06/01**

**註**：Phase II 與 Phase I 有2個月重疊期（2026/04-05），用於資料管道銜接與模型訓練準備，確保無縫過渡。實際執行期間為 M6-M17（共12個月）。

(1) 異常警示與路線偵測精準度
現行異常偵測主要依賴規則式閾值設定，無法即時因應路況、氣候與多國物流變數。系統難以區分「可容忍延誤」與「潛在欺詐繞路」，造成誤報率偏高與人工干預頻繁。為提升精度與時效性，本階段將導入 AI 模型進行時序分析與警示優化，降低誤報並提升警示反應速度。

(2) 感測器能耗與採樣頻率問題
Arviem IoT 感測器在長途運輸中常受限於電池壽命與網路頻寬。現行固定採樣間隔（每 10 分鐘一次）無法兼顧資料完整性與能耗效率，導致資料冗餘或電池提早耗盡。本階段將開發以強化學習（RL）為核心的智慧採樣策略，根據環境變化自動調整採樣頻率，平衡電力消耗與資料品質。
...
1. **目標與成功標準**
   ...
   - 建立異常行為預測與警示模型，提升警示精準度與時效性。
   - 發展智慧採樣機制，提升資料完整率 ≥ 95%，電池壽命延長 ≥ 40%。
   - 實現跨感測節點的警示整合與自動報告，減少人工介入 ≥ 50%。
   - 實際部署於 Arviem 實驗貨櫃，完成場域測試與聯合驗證。
2. **資料取得與整合**

   - 整合 IoT 感測器 GPS、溫溼度、震動、電壓、RSSI 等多維資料。
   - 結合外部交通、氣象資料，建立訓練用多維時序資料集。
   - 進行資料清洗、時序同步與異常樣本標註。
3. **AI 模型建置與訓練**

   - 應用 LSTM／Transformer 模型預測路線異常與潛在欺詐行為。
   - 採用 DQN / SAC 強化學習演算法設計智慧採樣策略。
   - 監控模型收斂與效能，確保在多路線、多環境條件下穩定運作。
4. **模型優化與測試**

   - 透過模擬與實測比較 Baseline 規則引擎與 AI 模型效能。
   - 驗證主要 KPI：F1 ≥ 0.85、誤報率 ≤ 15%、資料遺失率 < 5%。
   - 評估電池壽命延長率與資料品質提升幅度，完成性能驗證報告。
5. **部署與展示**

   - 部署於 Arviem 感測節點，實測智慧採樣與異常警示整合效能。
   - 展示「異常事件預測儀表板」與「自適應採樣壽命預測系統」。
   - 完成聯合測試與 ITRI 技術驗證報告，作為 Phase III 多智能體整合基礎。

---

### **成果預期與影響**

本階段將建立從異常偵測到採樣決策的閉環 AI 系統，透過智慧警示與能耗優化，
有效提升整體運輸監測效能與營運可持續性。完成後可使 Arviem 客戶即時掌握貨況、降低欺詐風險、延長感測器壽命並降低維運成本。

- B1 警示優化與路線異常預測 (15%) – ItracXing 準旺科技（M6-M12）
- B2 IoT 智慧採樣策略優化與部署 (19%) – ItracXing 準旺科技（M6-M14）
- B3 聯合驗證與測試 (5%) – ITRI + 學研中心（M15-M17）

### **Phase III：多智能體部署與國際展示（6 個月）— 開始日期：2027/06/01**

**註**：Phase III 實際執行期間為 M18（2027/06），總計畫期為18個月（2026/01/01 - 2027/06/30）。

(1) 多智能體協作與監控架構
為因應跨感測節點、不同航線與貨品環境條件的動態變化，需建立多智能體（Multi-Agent）協作平台。各智能體負責不同任務，如異常偵測、環境預測、採樣決策與資料整合。此平台可動態分配資源、平衡能耗、即時交換異常資訊，確保跨區域運輸的穩定性與可靠度。

(2) 聯邦學習與資料隱私
因應歐盟與國際客戶對資料隱私的嚴格要求，將導入**聯邦學習（Federated Learning）**架構，使各 Arviem 感測器與客戶節點可在不共享原始資料的前提下，進行模型協同訓練。此機制可兼顧 AI 準確度與數據主權，並支援跨國法規合規性。

(3) 實際部署與展示
本階段將整合 Phase II 成果（異常警示與智慧採樣模型），完成雲端-邊緣協同運行架構，實際部署於 Arviem 國際貨櫃與 IoT 節點。同時進行國際展示與行銷推廣，參與運輸與物流國際論壇，展示台灣 AI 技術於供應鏈數據智能化的領先應用。
...
1. **目標與成功標準**
   ...
   - 建立多智能體協作框架，支援多任務即時協同（警示、採樣、環境監控）。
   - 導入聯邦學習技術，確保跨客戶資料隱私保護。
   - 完成雲端與邊緣節點模型整合，實現多模型同時運行。
   - 國際展示與推廣：於主要物流會議展示系統成果，促進產業導入。
2. **資料取得與整合**

   - 整合 Phase I–II 產生之 AI 模型、感測器資料與警示紀錄。
   - 將環境條件（濕度、溫度、震動）、路線 GPS 與 Fraud 預測結果統合進多智能體環境。
   - 建立跨節點通訊與狀態同步模組。
3. **多智能體協作與訓練**

   - 以 RL-based Agent 與 Task Scheduler 設計「任務分配機制」。
   - 定義 Agent 角色（監控、採樣、警示、能源管理），建立通訊協定與決策層級。
   - 模擬多貨櫃／多環境條件之協作情境，驗證反應時間與資源利用效率。
4. **聯邦學習 POC 與模型同步**

   - 實作基於 Federated Averaging 的跨節點訓練流程。
   - 測試各客戶端（如歐洲咖啡豆貨櫃）與雲端模型參數同步效率。
   - 驗證模型精度下降 ≤ 5% 即可維持隱私合規。
5. **生產部署與展示**

   - 進行邊緣運算部署與雲端監控 API 整合。
   - 建立 API、文件與儀表板，支援管理單位查詢與系統維護。
   - 完成驗收測試與 ITRI 聯合報告。
   - 於國際運輸與物流論壇展示成果，推動市場滲透與國際合作。

---

### **成果預期與影響**

Phase III 完成後，系統將具備「分散協作、隱私保護、即時預警」的 AI 供應鏈監控能力。整合異常警示、環境監測與智慧採樣決策，可支援 Arviem 客戶即時掌握運輸風險並主動調整貨櫃配置。同時透過國際展示與聯邦學習驗證，展現台灣 AI 技術在全球供應鏈的落地價值與品牌影響力。

- C1 多智能體協作與管控（不含 DB 接入）(8%) – ItracXing 準旺科技（M18）
- C2 聯邦學習 POC (7%) – ItracXing 準旺科技 + 學研中心（M18）
- C3 生產部署 (6%) – ItracXing 準旺科技（M18）
- C4 API 與文件 (3%) – ItracXing 準旺科技（M18）
- C5 驗收與交付 (2%) – ItracXing 準旺科技 + ITRI（M18）

**註**：Phase III 集中於M18完成整合、驗證與交付工作，基於Phase I-II累積成果進行快速整合部署。
...
gantt
    ...
    %% -------------------------
    %% Phase II: 2026-06-01 ~ 2027-05-31 (12m, M6-M17)
    %% 註：2026/04-05 為重疊準備期
    %% -------------------------
    section Phase II: AI警示優化與智慧採樣（M6-M17）
    B1 警示優化與路線異常預測                       :b1, 2026-06-01, 6m
    B2 IoT 智慧採樣策略（DQN／SAC）                 :b2, 2026-06-01, 8m
    B3 模型訓練與資料整合                          :b3, 2026-07-01, 7m
    B4 模型優化與性能比較（Baseline vs AI）          :b4, 2026-11-01, 4m
    B.1 Checkpoint（Phase II 前期完成）             :milestone, b1_cp, 2026-12-31, 0d
    B5 聯合驗證與測試（ITRI＋學研中心）             :b5, 2027-03-01, 3m
    B.2 Checkpoint（Phase II 完成+ITRI報告）        :milestone, b2_cp, 2027-05-31, 0d

    %% -------------------------
    %% Phase III: 2027-06-01 ~ 2027-06-30 (1m, M18)
    %% -------------------------
    section Phase III: 多智能體部署與展示（M18）
    C1 多智能體協作整合                             :c1, 2027-06-01, 1m
    C2 聯邦學習 POC 驗證                             :c2, 2027-06-01, 1m
    C3 生產部署與上線                               :c3, 2027-06-01, 1m
    C4 API 文件交付                                  :c4, 2027-06-01, 1m
...
- **2026/01–2026/05（Phase I，M1-M5）**：完成 A1–A5 原型與指標測試，包含自然語言查詢、電池異常偵測、Kalman降噪、MCP資料庫整合、自動報告系統（查核點 A.1）
- **2026/06–2026/12（Phase II 前期，M6-M12）**：完成 B1 警示優化與路線異常預測、B2 IoT智慧採樣策略開發（查核點 B.1）
- **2027/01–2027/05（Phase II 後期，M13-M17）**：完成 B3 聯合驗證與測試，ITRI聯合驗證報告（查核點 B.2）
- **2027/06（Phase III，M18）**：完成 C1–C5 多智能體協作、聯邦學習POC、生產部署、API文件、驗收與交付（查核點 C.1；結案驗收）

### 預定查核點說明

- **A.1（2026/05底）—Phase I結束**：

  - 異常偵測準確率≥85%；Kalman降噪SNR提高≥20%
  - MCP資料庫整合完成，支援即時查詢
  - 自動報告與高階警示彙報系統上線
  - 交付：系統原型、技術文件、測試報告
- **B.1（2026/12底）—Phase II前期結束**：

  - 警示優化模型訓練完成，F1-score≥0.85
  - IoT智慧採樣策略開發完成，實驗室測試通過
  - 交付：模型訓練報告、採樣策略文件
- **B.2（2027/05底）—Phase II結束**：

  - 警示誤報率≤15%（較基準降低≥40%）
  - 電池壽命延長≥40%（實際場域驗證）
  - ITRI聯合驗證報告完成
  - 交付：ITRI驗證報告、場域測試數據、性能對比分析
- **C.1（2027/06底）—Phase III結束（結案驗收）**：
...
| --------------- | --------- | ----------------------------- | ---------- | ------------- | ------------ |
| **Phase I**   | M1-M5   | NLQ + 異常偵測 + Kalman降噪 | 40%      | 4,960K      | —         |
| **Phase II**  | M7-M12  | RL警示 + 採樣優化 + 驗證    | 35%      | 4,338K      | —         |
...
於Phase I結束（2026年6月）進行**重大審查**，需達成**4項指標中至少3項**方可繼續Phase II/III：
...
- **通過（GO）**：達成3/4項指標 → 繼續Phase II/III，按原計畫撥付
- **有條件通過**：達成2/4項指標 → 提交改善計畫，Phase II預算保留10%，M9再審查
- **暫停（NO-GO）**：達成<2項指標 → 終止計畫或大幅調整範圍，Phase III預算不撥付
...
| 國際輸出合作（歐洲場域）         | Arviem AG              |            1,500 | 將 AI-RAN 整合方案輸出至 Arviem 歐洲客戶場域（如荷蘭港口），作為跨境驗證合作             |
...
- 專長：強化學習 (DQN)、PyTorch、模型優化、Kalman濾波
- 本業年資：3年
- 參與分項計畫及工作項目：
  - Phase II：B1 DQN 警示優化主要開發者
...
- 最高學歷：國立勤益科技大學 資訊工程所 碩士（20__年）
- 主要經歷：
  - ITracXing IoT工程師（2021–現在）4年
  - [前公司] 系統工程師（20__–2021）
- 專長：Python、Node.js、IoT系統整合、韌體部署、API整合
- 本業年資：6年（IoT系統）
- 參與分項計畫及工作項目：
  - Phase II：B3 IoT採樣策略優化與部署主要開發者
  - Phase I/II：MCP資料庫整合與API開發
- 投入月數：12個月
...
| IoT 系統工程師 × 2      |          80 |                     16 |                1,280 | 8 個月 × 2 人                           |
...
| 開發板與通訊模組（MCU/NB-IoT/BLE）           | 片   |   10 |   25 |     250 | IoT 採樣測試、韌體原型（Phase II–III）   |
...
| microSD/USB 儲存媒體                         | 個   |   20 |    2 |      40 | IoT裝置資料快取與記錄                     |
| 感測器備品與替換件                           | 組   |   20 |   12 |     240 | 場測維護與誤差校正備援                    |
| 小量打樣 PCB                                 | 片   |    6 |   40 |     240 | IoT裝置連接板打樣與調整                   |
...
| AI 開發助理工具（IDE外掛/協作）       | 月租     |    6 |   18 |     108 | 開發者生產力工具（例如：AI程式輔助、評審加速、文件生成）          |
| 程式碼安全與品質掃描（SAST/依賴檢查） | 月租     |    5 |   18 |      90 | 供應鏈安全與弱點掃描、合規檢查                                    |
| 小計                                  | —       |   — |   — |     738 | 金額單位：千元；**僅保留推論與監控/部署必要服務，並增列開發工具** |

**雲端資源與開發工具策略（修訂版）**：

- **Phase I（M1-M5）**：以本地GPU/伺服器進行訓練與資料處理；雲端僅支援 LLM 推論、Demo/測試用容器與基礎監控；導入 AI 開發助理與 SAST 工具並建立 CI/CD。
- **Phase II（M6-M17）**：持續以本地設備進行 RL/時序模型迭代與 A/B 測試；雲端僅支援 Pilot 容器化服務與可觀測性；資料備援以 NAS 與離線異地備份為主（不使用雲端儲存）。
- **Phase III（M18）**：完成生產部署與驗收；雲端資源僅作為最小可用服務（Minimal Viable Service）的託管與監控，無大規模訓練需求。
- **成本控制機制**：停用雲端 GPU 權限；所有資源採按量計費與預算上限；非工作時段自動關閉；重大變更需經 PR/審核；月度成本檢視與預算警示。
...
| 項目                             | 單位 | 數量 | 單價 | 第1年度 | 用途說明                                                                 |
| ---------------------------------- | ------ | -----: | -----: | --------: | -------------------------------------------------------------------------- |
| GPU 維護/延伸保固與風扇模組汰換  | 年   |    1 |  500 |     500 | 硬體維護合約與零件汰換；**以本地訓練為主，確保長期穩定**                 |
| NAS 維護/RAID 硬碟汰換與延伸保固 | 年   |    1 |  450 |     450 | 磁碟模組/控制器維護與保固；**以本地儲存/備援為主**                       |
| 伺服器維護/延伸保固與備品        | 年   |    1 |  450 |     450 | 電源/記憶體/備品汰換；**確保本地關鍵設備可靠性**                         |
| 小計                             | —   |   — |   — |   1,400 | 此小計對應彙總表「4.設備維護費」；**支援以本地設備為主之訓練與儲存策略** |

### 3-4 合計（3-1 + 3-2）

| 合計 |  |  |  | 1,818 | 金額單位：千元；**已反映雲端與設備資源調整** |
...
| ---------------------------- | ------------------------------------- | --------------------- | ----------- | ------------------------------------------------------------------------------------- |
| 工研院 AI 中心             | RL驗證與系統測試                    | 現金支援            | 1600      | • RL模型驗證報告<br>• 系統性能測試報告<br>• 技術諮詢（12人月）                   |
| Arviem AG                  | 場域測試與資料                      | 實物貢獻 + 技術支援 | 800       | • 歐洲場域測試環境<br>• IoT設備與資料存取<br>• 技術協作（共同開發）              |
...
| 開發者工具與教育訓練       | AI開發助理、程式碼安全、內訓/工作坊 | 服務費/訓練費       | 1,062     | • AI工具導入與教育訓練計畫<br>• 內部技術工作坊（每季）<br>• 開發者生產力提升報告 |
| **合計**                   | —                                  | —                  | **6,262** | **已優化預算分配並提供詳細交付成果（增列開發者工具與教育訓練）**                    |
...
| 項目           | 單位                                                        | 數量 | 單價 | 第1年度 | 用途說明                                                                                                                                                                                                                                                                                                                                                                                                         |
| ---------------- | ------------------------------------------------------------- | ------ | ------ | --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 合作研發與驗證 | 工研院 AI 中心／Arviem AG／台灣–阿姆斯特丹大學 AI 合作中心 | 1    | —   | 6,262   | **已優化預算結構**：<br>(1) 工研院 AI 中心：強化學習與多智能體模組共同研發、系統性能驗證與測試報告（現金600K）。<br>(2) Arviem AG：歐洲場域測試、IoT冷鏈數據驗證與跨境資料回傳（實物貢獻1,800K，提供設備與資料存取）。<br>(3) 台灣–阿姆斯特丹大學AI中心：聯邦學習、AI倫理與可解釋模型協同研究（學術合作800K）。<br>(4) 外部顧問、專利申請與第三方驗證（2,000K）。<br>(5) 新增：開發者工具與教育訓練（1,062K）。 |
...
| 項目             | 單位 | 數量 | 單價 | 第1年度 | 用途說明                     |
| ------------------ | ------ | -----: | -----: | --------: | ------------------------------ |
| 國內專利申請獎勵 | 件   |    2 |   30 |      60 | 供應鏈異常偵測與智能採樣優化 |
...
| 會計科目                |     補助款 |     自籌款 |       合計 |     占比 |
| ------------------------- | -----------: | -----------: | -----------: | ---------: |
| 1.人事費                |      6,270 |      6,260 |     12,530 |    50.5% |
| 2.消耗性器材及原材料費  |        810 |        810 |      1,620 |     6.5% |
| 3.設備使用費/雲端租賃費 |        909 |        909 |      1,818 |     7.3% |
| 4.設備維護費            |        700 |        700 |      1,400 |     5.6% |
| 5.技術引進/合作研發費   |      3,131 |      3,131 |      6,262 |    25.3% |
| 6.國內差旅費            |        200 |        200 |        400 |     1.6% |
| 7.專利申請獎勵金        |         80 |         80 |        160 |     0.6% |
| 8.應急準備金與風險管理  |        300 |        300 |        600 |     2.4% |
| **總開發經費**          | **12,395** | **12,395** | **24,790** | **100%** |
| **百分比**              |  **50.0%** |  **50.0%** |   **100%** |       — |
...
3. 預測為審慎評估情境 (Cautious forecast)，顯示對市場反應和客戶導入持高度信心。
...
| 項目           | 2026 (USD) | 2027 (USD) | 2028 (USD) | 2029 (USD) |
| ---------------- | ------------ | ------------ | ------------ | ------------ |
| 總收入         | $93,000    | $752,000   | $1,142,000 | $1,560,000 |
...
2. **服務收入重要性提升：** 儘管硬體佔比高，但服務相關收入 (b+c) 的佔比呈穩定微幅上升趨勢 (從 19.35%增長至 23.08%)。這與應對措施中提到的**「建立多元化收入來源（顧問服務、技術授權）」**相呼應。服務收入（b） 代表 SaaS 或 LaaS（租賃即服務）的經常性收入，雖然在總收入中佔比較小，但由於其高毛利特性（硬體毛利率 50%，服務毛利通常更高），其對公司利潤和估值的貢獻可能比其營收佔比更為重要。
3. **成長驅動因子：**
...
## 審慎預估情境（Cautious forecast）

1. **2026：ARR NT$2.98M**（首批 IoT 追蹤裝置與 Smart TOTE 銷售，早期客戶 Arviem&Vector 簽約，市場反應優於預期）
2. **2027：ARR NT$24.06M**（IoT 追蹤裝置與 Smart TOTE&TC605 + LaaS IoT Devices 銷售大幅成長，服務收入倍增，新增大型客戶）
3. **2028：ARR NT$36.54M**（多國市場擴展，與國際企業級客戶簽約，產品線擴充）
4. **2029：ARR NT$49.92M**（全球市場滲透率提升，多元化產品與服務帶動高成長）
