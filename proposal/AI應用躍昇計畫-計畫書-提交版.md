...
### **Phase I: AI驅動互動即時報告服務（Arviem 客戶） (4.5 months, 27%) — Start Date: 2026/01/01 **
...
1. **目標與成功標準**
   ...
   - 提供即時可視化介面與查詢服務，反應時間 < 2 秒。  
   - 建立電池壽命關鍵指標 (Battery Health Index, BHI)，涵蓋溫度、容量與電壓變化。  
   - 週/月報自動生成，減少人工報告製作時間 ≥ 50%。  

2. **資料取得與整合**
   - 整合電池電壓 (V)、剩餘容量 (%)、溫度 (°C)、感測器傳輸頻率 (RSSI/Tx Interval) 等資料。  
   - 導入初步清理與平滑機制（如移動平均與卡爾曼濾波）。  
...
- B1 DQN警示優化 (15%) – ItracXing 準旺科技 + ITRI(驗證)
- B2 自適應Kalman調校 (8%) – ItracXing 準旺科技
- B3 IoT採樣策略優化與部署 (19%) – ItracXing 準旺科技
...
- **2026/05中–2026/12（Phase II）**：完成 B1 訓練與A/B測試、B3 採樣策略優化與部署（查核點 B.1、B.2）  
