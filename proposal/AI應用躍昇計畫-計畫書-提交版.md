...
全球智慧物流監控與可追溯需求快速成長，2030 年運輸與物流市場規模預估達 NT$250.2 兆。本計畫由 ItracXing 與瑞士 Arviem 共同推動，聚焦「感測器可靠度不足、報表產出耗時、環境敏感貨品風險難以及時掌握、路線繞行與疑似竊盜無法即時辨識」四大痛點，分別建置 BHI/RUL 電池健康預測、NLQ＋AI 自主式報告、環境異常偵測與路線／竊盜偵測四項 AI 模組。18 個月內完成 **至少 1 家國際＋1 家國內場域**（國際場域：Arviem；<mark> 國內場域：Simique(TBD)）</mark> ，達成感測器可靠度 ≥95%、風險回應時間縮短 ≥40%、溫濕度敏感貨品環境違規率下降 ≥30%、竊盜／未授權開啟事件偵測 F1 ≥0.85，並建立可輸出的 AI-MaaS 服務模式，預估結案後三年累積產值約 <mark> NT$1.2 億元 </mark> ，提升台灣在 AI 智慧物流領域之國際能見度。
...
| 商業 | 營運效率與成本優化、可實收之訂閱與專案收入基礎 | 週/月報與事件分析等人工報告工時每月減少 **≥200 小時**；溫濕度敏感貨品環境違規率下降 **≥30%**；竊盜／未授權開啟事件偵測 F1 **≥0.85**，無效運輸與理賠相關損失降低 **≥10–15%**；結案時取得 **≥2 件** 國內外 LOI/POC，並完成 **至少 1 家國際＋1 家國內場域**（國際場域：Arviem； 國內場域：Simique(TBD)） 之付費或準商用驗證，作為結案後三年累積 **≥NT$120M** 產值之可查核基礎。 |
| 產業 | 場域示範、在地生態系與跨域人才培育 | 完成 **至少 1 個** 具代表性的 AI 導入示範場域（如國際海運／溫濕度敏感貨品／電子組裝供應鏈），形成可複製之導入藍圖與 SOP；培育或提升具「AI＋物流／製造」跨域能力之研發與實務人才 **≥6 名**，其中女性研發人力 **≥1 名**；透過技術工作坊、產業論壇與標竿案例分享，帶動國內系統整合商、電信與設備廠之合作機會。 |
...
| 社會／ESG | 供應鏈韌性強化、節能減碳與中小企業示範 | 透過提前預警與路線優化，關鍵異常事件之風險回應時間縮短 **≥40%**，提升跨境供應鏈韌性；結合自適應採樣與路線最佳化，推估三年累計 CO₂ 排放量減少 **約 10–15%**（以里程／電池使用效率計算），折算約 **80+ 噸 CO₂e**；完成 **1–2 個** 台灣中小企業導入示範（可含物流／製造／溫濕度敏感貨品業者），形成兼顧資料治理、AI 導入與商業模式的整合示範案例，可供後續政策與產業推廣參考。 |
...
- 場域規劃
  - 國際場域：Arviem（瑞士；國際驗證場域）
  - 國內場域：Simique(TBD)（台灣；合作對象洽談中）
...
2. <mark> ** 衍生投資與人力效益（千元；非本期核心 KPI）** </mark>

| 成果項目                          | 執行前 | 第 1 年度 | 第 2 年度 | 佐證方式                         |
|-----------------------------------|-------:|----------:|----------:|----------------------------------|
| <mark> 申請／核定專利數 </mark>                |      0 |         2 |         3 | 專利申請書及審查進度文件         |
...
- 產品與商業模式：  
  ...
  - 完成至少 1–2 個可複製之國際場域示範案例（國際場域：Arviem；國內場域：Simique(TBD)），作為往後標竿案例。
...
<mark> 3. **營收預測與收入結構（2026–2029）** </mark>
...
---
6. **國際展會策略（CES 2026 概念驗證節點）**

定位：目前 AI-MaaS/各模組仍處「概念與設計階段」。CES 2026 以「痛點 → 概念架構 → 原型交互（Mock/Replay）」為核心，驗證需求優先級、蒐集規格回饋、鎖定共創／早期試用夥伴。

6.1 展出焦點（四大痛點概念化呈現）
- 感測器可靠度與電池剩餘壽命不可預測 → BHI/RUL 計算流程示意＋原型儀表板（模擬衰退曲線）
- 報告產出耗時 → NLQ＋事件報告互動稿（Mock Query→生成報告對比人工流程）
- 溫/濕度敏感貨品異常難提前介入 → 異常類型範例＋30–60 分鐘預警邏輯流程圖
- 路線繞行／竊盜風險辨識延遲 → GPS/LTE/NTN 資料流＋Padlock/壓力事件整合示意與回放

6.2 Demo 與互動形式（均為原型/模擬）
- 3–5 分鐘循環原型 Demo：1）環境異常預警模擬 2）路線偏離＋疑似開箱事件回放 3）ETA 誤差校正概念流程
- Smart TOTE NTN 概念展示：影片＋記錄回放（海上／離線補傳情境）
- 雙軌備援：即時原型操作＋備援錄製（故障時 10 秒內切換）

6.3 回饋與需求蒐集
- 展前：寄送「痛點優先級＋模組興趣」問卷（目標有效回收 ≥30 份）
- 展中：Feedback Corner（5 題速填；功能缺口／資料可得性／計價接受區間）
- 展後：48 小時內寄出「個別痛點摘要＋共創工作坊邀請」；排程深度會議（意願名單 ≥60% 進入排程）

6.4 目標客群與轉換漏斗（概念→共創）
- 客群：國際貨代／3PL、溫濕度敏感與高價值貨品品牌、電信／衛星、系統整合商
- 展前：鎖定 ≥40 家；排定 ≥15 場 1:1 概念會談；準備 2–3 組 ROI 估算框架（假設 vs 後續驗證指標）
- 展後：提出「Concept Validation Pilot（3 個月）」方案；遴選 1–2 家首波國際共創夥伴（非正式商用）

6.5 在地與政策連結
- Taiwan AI Logistics 概念聯合展示：串連國內電信／SI 作為「台灣技術中心」敘事
- 展後工作坊：痛點→資料需求矩陣→導入成熟度評估表（支援政策推廣／中小企業自評）

6.6 差異化亮點（以架構與設計主張為核心）
- Smart TOTE 設計：GPS/LTE/NTN 三通道＋BLE Padlock＋壓力事件資料融合管線（設計圖）
- NTN 覆蓋策略：離線緩衝＋延遲補傳機制（訊息可靠度模型概念）
- AI-MaaS 架構：模組化（BHI/RUL／環境異常／路線與竊盜／NLQ 報告）→ 共用治理層（審計／存取）
- 低門檻試用思路：訂閱制＋情境 KPI 導入路徑；90 日「驗證假設→調整規格」

6.7 CES 概念階段量測指標（精簡版）
- 1:1 會談 ≥15
- 原型 Demo 套數 ≥3
- 展後 3 個月 LOI／POC（概念/共創意向）≥5
- 首波共創國際夥伴 1–2 家
- 媒體／專業曝光 ≥5 篇
- Pilot→深化合作（6 個月內）轉換率 ≥30%
- 問卷（展中＋展後）有效樣本 ≥50 份

6.8 與整體計畫 KPI 連動（「概念→準備度」層）
- 客戶基礎：為結案後三年 ≥10 家付費／長期試用建立早期名單池
- 裝置規模：形成 Smart TOTE 部署計畫（預留至 ≥1,000 台擴張路徑）
- 國際營收占比：以首波共創夥伴作為 70% 國際占比的加速引線
- 媒體／白皮書：收斂設計回饋→支援 2–3 件專利撰寫與 1 件 PCT 申請素材

6.9 風險與緩解（概念階段特化）
| 風險 | 兆候 | 緩解措施 |
|------|------|----------|
| 展前排程不足 | 回覆率低 | T‑60 名單凍結；T‑45 雙語 Pitch 定稿；每日補位追蹤 |
| 原型不穩定 | 互動失效／延遲高 | 全部模組同時備援錄製；Mock API 離線封包快取 |
| 轉換意願低 | LOI/POC 低於預期 | 即場預約深度會議＋限期 Concept Pilot（折扣＋專屬 KPI） |
| 回饋不足 | 問卷回收 < 目標 | 即填抽獎／QR 動線優化／題數 ≤5／現場人員引導 |
...
- 即時預約深度會議覆蓋 ≥40% 目標名單
- 原型與備援切換零重大故障
- 功能缺口／資料可得性回饋集中度（Top 5 痛點收斂度）提升

（註：所有「即時／預測／自動」字樣於 CES 現場均以「原型概念展示／模擬數據」標示，避免誤導產品成熟度；後續商業文件中清楚列出「設計中／未量產／需場域驗證」標示。）
...
- BHI／RUL 預測準確度驗證（分項 A／B）：  
  <mark> - 場域：（國際場域：Arviem；<mark> 國內場域：Simique(TBD)  100 台裝置，12 個月資料。 </mark> 
