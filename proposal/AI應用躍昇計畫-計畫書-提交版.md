...
全球智慧物流監控與可追溯需求持續攀升，<mark>2030 年運輸與物流市場規模估達 NT$250.2 兆</mark>。ItracXing 與瑞士 Arviem 協作開發 AI 驅動之智慧供應鏈監控與風險預警系統，聚焦四大痛點：<br>1) 感測器可靠度—透過 AI 化電池健康指標 (BHI) 與剩餘壽命 (RUL) 預測模型，實現預測性維護，可靠度由 90% 提升至 ≥95%；<br>2) 即時報告—導入 NLQ + 多智能體語意編排，AI 自主式生成週/月報與異常摘要，顯著縮短人工彙整並將風險回應時間減少 40%；<br>3) 環境風險—以溫/濕度異常偵測＋AI 自主式告警觸發 AI 報告為 MVP，並設定報告 SLA（P95 ≤ 2 分）與提前預警率目標；<br>4) 路線與竊盜偵測—融合 GPS、GeoFencing、鎖具/壓力感測（Sensor Fusion）以降低誤報並強化安全。預期異常偵測準確率提升 30%。


### 計畫結案時預期效益（摘要表）

以下為本計畫結案時於「技術、商業、產業、智財與輸出、社會」五大面向之核心效益指標與具體目標；其量測方法、基準值與追蹤頻率請參見「五、預期效益」及後文 KPI 表與驗證協議章節。

| 類別 | 核心指標（摘要） | 結案目標（具體且可查核） |
|------|------------------|--------------------------|
| 技術 | 感測器可靠度與預測能力、異常偵測與查詢效率 | 感測器可靠度（Availability）由 90% 提升至 ≥95%；BHI 預測準確率 ≥90%，RUL 預測誤差 MAE ≤10 天；環境異常偵測 F1 ≥0.88，路線偏離偵測 AUC ≥0.90、竊盜偵測 F1 ≥0.85；NLQ 查詢正確率 ≥92%，P95 延遲 <45 秒，整體風險回應時間縮短 ≥40%。 |
| 商業 | 營運效率與成本優化、客戶價值提升 | 週/月報與事件分析等人工報告工時每月減少 ≥200 小時；冷鏈／環境違規率下降 ≥30%，無效運輸與理賠相關損失降低 ≥10–15%；結案時取得 ≥2 件國內外 LOI/POC，並落實至少 1 家國際與 1 家國內場域之付費或準商用驗證，為結案後 3 年累積 ≥NT$120M 產值之基礎。 |
| 產業 | 場域示範與跨域人才培育 | 完成至少 2 個具代表性場域（國際海運／陸運或冷鏈）之 AI 導入示範案例，形成可複製之導入藍圖與 SOP；培育/提升具 AI＋物流／製造跨域能力之研發與實務人才 ≥6 名（含女性研發人力至少 1 名），並透過技術工作坊／分享會擴散應用場景。 |
| 智財／輸出 | 智慧財產與 AI 模型商轉化 | 結案前完成 2–3 件專利申請（含至少 1 件具國際佈局潛力之 PCT），並完成 1–2 項核心模組之電腦程式著作權登記；建立可商轉之 AI-MaaS / SaaS 模式（含 BHI/RUL、環境異常、路線異常與 NLQ 報表等模組），並完成至少 1 個對外技術白皮書或開源工具（如資料 Schema / NLQ Prompt 套件），支援國際市場推廣。 |
| 社會 | 供應鏈韌性、ESG 與中小企業示範 | 透過提前預警與路線優化，關鍵異常事件風險回應時間縮短 ≥40%，強化跨境供應鏈韌性；結合自適應採樣與路線最佳化，推估碳排量降低 ≥10–15%；完成 1–2 個中小企業導入示範（可含國內物流／製造業者），形成「資料治理＋AI 導入＋商業模式」整合示範路徑，作為後續政策與產業推廣參考。 |
...
| **痛點二｜報告產出耗時且缺乏互動（人工彙整、決策延遲）** | **B. 按需報告與即時查詢（NLQ）** | 自然語言查詢（NLQ）、NL-to-SQL、LLM 報告生成 | 讓管理階層與營運人員可用自然語言下指令取得數據與報告，將報告產出由 **「人工數小時 → 自動數秒」**，並將 NLQ P95 延遲目標調整為 <180 秒、查詢正確率 ≥90%。 |
...
**B1. 目標與成功標準**
- 報告製作時間下降 ≥ 50%；NLQ P95 延遲 < 180 秒；正確率 ≥ 90%。
...
> 本節為新增說明段落，補充國內外市場規模與本計畫三年產值推估，所有數據皆以可查證之外部資料為來源，並於文中說明計算假設與公式，供審查委員作為背景參考，不作為本期 KPI 之直接查核指標。
...
> 註：本表所列指標與前述「技術驗證協議」、「預定進度表與查核點」及里程碑（A.1／B.1／B.2／C.1）一一對應。其中 NLQ P95 延遲 <45 秒 為期末目標值；文中提及之 <180 秒 為 M1–M6 初期壓力測試與 Go/No-Go 參考基準。警示誤報率 ≤15%、環境異常 F1 與路線／竊盜 F1 等數值亦與技術驗證協議之目標設定一致。
...
【說明】為確保本計畫所主張之 AI 技術效益（如 BHI/RUL 預測準確度、環境／路線異常偵測與警示優化等）具備「可量測、可查核」之客觀依據，本節針對關鍵技術指標規劃具體之驗證樣本、評估方法與時程，並與前述 KPI 及里程碑一一對應，作為後續審查與結案驗證之依據。

#### 一、BHI／RUL 預測準確度驗證（分項計畫 A）

**1. 驗證樣本與場域**

- 設備數量：共 100 台 IoT 感測器。
- 場域型態：Arviem 現有實際場域（海運／陸運混合，可含部分冷鏈）。
- 資料期間：連續 12 個月歷史資料。
- 收集指標：電壓、溫度、傳輸頻率、使用模式、維護／故障事件等。

**2. 驗證方法與模型概述**

- 建立訓練／驗證資料集：
  - 訓練資料：前 9 個月資料。
  - 驗證資料：後 3 個月資料＋獨立測試子集（約 50 台設備）。
- AI 模型：以時序深度學習模型（如 LSTM／TFT）結合 Kalman 濾波與生存分析（RUL 模型），建構電池健康指標（BHI）與剩餘壽命（RUL）預測；同時保留傳統門檻／規則作為對照組。
- 驗證方式：
  - BHI：預測健康狀態 vs 實際健康／故障標註之分類準確率。
  - RUL：預測剩餘壽命 vs 實際故障時間之 MAE；計算提前預警率與誤報率。
  BHI 與 RUL 預測結果亦將作為分項計畫 B 之 NLQ／自動報告模組的核心指標來源，供後續查詢與高階彙報使用。

**3. 成功標準與對應 KPI**

- BHI 預測準確率：≥ 90%。
- RUL 預測誤差（MAE）：≤ 10 天。
- 提前預警率：≥ 85%（於故障前 14 天內發出預警）。
- 誤報率：≤ 15%（避免過度預警導致維護成本過高）。

上述指標將以獨立測試集與場域驗證報告佐證，並對應 KPI 表中之「感測器可靠度」、「BHI/RUL 預測準確度」等指標。

**4. 驗證時程（對應里程碑）**

- M1–M5：歷史資料彙整、特徵工程與初版 BHI/RUL 模型（對應 A.1）。
- M6–M12：模型訓練與內部驗證，提出初步成效報告。
- M13–M15：獨立測試集與實際場域驗證，完成技術驗證報告（對應 B.2）。
- M16–M18：上線後持續追蹤與調校，納入結案報告。

#### 二、警示優化機制性能驗證（分項計畫 C／D）

**1. 驗證設計與樣本**

- 基準系統：既有固定閾值規則引擎（Rule‑based Alerts）。
- 目標系統：動態警示優化機制（Learning‑to‑Rank＋自適應閾值＋異常評分融合）。
- 測試資料集：Arviem 提供之約 18 個月歷史資料（含環境異常與路線／竊盜相關事件）。
- 線上 A/B 測試：系統上線後採 50/50 流量分流，連續觀察 3 個月。

**2. 對比指標與改善目標**

| 指標                     | 規則引擎 Baseline | 動態優化系統 Target | 改善目標 |
|--------------------------|-------------------|----------------------|----------|
| 警示誤報率（含天候／交通過濾） | 約 25%            | ≤ 15%               | -40%     |
| 漏報率                   | 約 8%             | ≤ 5%                 | -37.5%   |
| F1-score                 | 約 0.72           | ≥ 0.85               | +18%     |
| 關鍵警示回應時間         | 約 60 分鐘        | < 30 分鐘           | -50%     |
| 使用者滿意度（NPS）      | 基準值            | +20 分以上           | —        |

上述指標分別對應計畫中「環境異常偵測 F1」、「路線／竊盜偵測 F1」、「警示誤報率」、「關鍵警示回應時間」與「營運人員滿意度」等 KPI。

**3. 驗證文件與交付成果**

- 系統性能與 A/B 測試報告（預計 M15 前完成）。
- Arviem 實際場域驗證報告（預計 M18 前提供），說明在實際運輸任務中的警示成效與案例。
- 視需求邀請第三方單位出具獨立測試／驗證報告，作為結案查核與後續商轉之佐證文件。
...
| **NLQ P95 延遲**                                     |                           < 180 秒（壓測基準） |             **< 180 秒** | 壓力測試：P95 latency 指標（含 200 並發、1,000 筆查詢） | APM、壓測報告              | 週 / 月   |   |
...
| ---------------------------------- | ------------------------ | -----------------: | ------------------------------------------------------------------------------------------ |
| 智慧物流邊緣節點測試與驗證合作（延伸選項） | 外部研究機構 |            3,000 | 與外部研究機構合作進行智慧物流邊緣節點之試煉場域部署及性能驗證（結案後延伸，非本期KPI），建置測試節點與 AI 加速伺服器 |
| Edge AI Platform 共研投資（延伸選項） | ItracXing & Arviem     |            2,000 | 延伸本計畫 AI 模組至邊緣運算節點（結案後延伸，非本期KPI），以支援低延遲運算與智慧物流監控                     |
| 國際輸出合作（全球場域；延伸選項）         | Arviem AG              |            1,500 | 將智慧物流監控整合方案輸出至 Arviem 客戶場域（如主要國際港口），作為跨境驗證合作（結案後延伸，非本期KPI）             |

合計衍生投資金額：NT$6,500 千元

備註：上述智慧物流邊緣節點相關項目屬結案後延伸投資，非本期核心交付或 KPI，啟動條件為 AI‑MaaS 與 Arviem 合作成效達標。
...
邊緣運算延伸應用（共同研發帶動；結案後情境，視 AI‑MaaS × Arviem 成效啟動）
- 國內智慧物流邊緣運算相關產業鏈年產值（延伸選項；非本期KPI）：每年約 NT$30M
- 海外輸出（智慧物流監控整合專案與服務；延伸選項，非本期KPI）：三年累計約 NT$90M
...
- 三年累計總體產值（AI-MaaS + 邊緣運算延伸選項〈非本期KPI〉帶動，含上中下游）：約 NT$120M
- 出口比重：約 70%；內需貢獻：約 30%
...
| 邊緣運算相關專案/授權收入 (d，結案後延伸，非本期KPI) | $0          | $60,000      | $200,000       | $200,000       |
| **總收入 (a+b+c+d)**                | **$93,000** | **$812,000** | **$1,342,000** | **$1,760,000** |
| **ARR (NT$, 匯率 1:32)**            | **2.98M**   | **25.98M**   | **42.94M**     | **56.32M**     |

註：上述表中項目 d（邊緣運算相關專案/授權收入）為結案後延伸收入來源，非本期交付或績效目標。
...
> 本節為前述「六、風險評估與因應對策」之摘要版，將技術、資料、整合、人力與政策等主要風險類別及一級對策濃縮整理，供審查委員快速掌握重點；詳細矩陣、監控機制與關鍵人員備援規劃請參照第六節。
